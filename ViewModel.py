...
class Data3D:
    ...
    def __init__(self, img: ndarray):
        self.rawData = img
        self.grayScale8 = normalizeToGrayScale8(self.rawData)
...
class View3D:
    ...
    def getXSlice(self, x: int) -> ndarray:
        return resizeImage(self.data.grayScale8[x, :, :], self.displaySize)

    def getYSlice(self, y: int) -> ndarray:
        return resizeImage(self.data.grayScale8[:, y, :], self.displaySize)

    def getZSlice(self, z: int) -> ndarray:
        return resizeImage(self.data.grayScale8[:, :, z], self.displaySize)
